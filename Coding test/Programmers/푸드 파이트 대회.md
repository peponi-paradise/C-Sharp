...
1. 중앙의 물에 대한 값 `0`을 미리 추가한다.
2. 주어진 `food`의 값을 절반으로 나누어 양쪽 끝에 추가한다.
...
public class Solution
{
    public string solution(int[] food)
    {
        ...
        {
            AddDatas(ints, i, food[i]);
        }

        return string.Join("", ints);
    }

    private void AddDatas(List<int> datas, int foodIndex, int count)
    {
        int divCount = count / 2;
        for (int i = 0; i < divCount; i++)
        {
            datas.Add(foodIndex);
            datas.Insert(0, foodIndex);
        }
    }
}
```